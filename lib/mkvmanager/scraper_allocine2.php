 ...
 */
class MkvManagerScraperAllocine2 extends MkvManagerScraper
{
    /**
    ...
    public function searchMovies( $queryString )
    {
        ...
        foreach( $doc->movie as $movie )
        {
            ...
            $result->allocineId = (int)$movie['code'];
            ...
            $result->director = (string)$movie->castingShort->directors;
            $result->actors = explode( ', ', (string)$movie->castingShort->actors );
    ...
    public function getMovieDetails( $movieId )
    {
        ...
        $result->allocineId = (int)$doc['code'];
        ...
        foreach( $doc->casting->castMember as $person )
        {
            ...
            if ( (int)$person->activity['code'] == 8001 )
            {
                if ( (string)$person->role == '' )
                    continue;
                $personResult = new MkvManagerScraperAllocineActor();
                $personResult->name = (string)$person->person->name;
                $personResult->thumbnail = (string)$person->picture[0]['href'];
            ...
            {
                $personResult = new MkvManagerScraperAllocinePerson();
                $personResult->name = (string)$person->person->name;
                $personResult->thumbnail = (string)$person->picture[0]['href'];
        ...
        foreach( $doc->mediaList->media as $media )
        {
            ...
            elseif ( (string)$media['class'] == 'video' )
            {
                ...
                {
                    $trailerObject = new MkvManagerScraperAllocineTrailer();
                    $trailerObject->title = (string)$media->title;
                    $trailerObject->href = $trailerHref;
